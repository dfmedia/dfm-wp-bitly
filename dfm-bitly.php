...
class DfmBitly {
	...
	function __construct() {
		...
		add_filter( 'sharing_permalink', array( $this, 'short_sharing_link' ), 10, 3 );
	}
	...
	/**
	 * Filters the link to be shared
	 *
	 * @param $permalink  string passes the post permalink to be filtered
	 * @param $post_id  int passes the post ID from the post object
	 * @param $social_network  string name of social network
	 *
	 * @return mixed|string
	 */
	function short_sharing_link( $permalink, $post_id, $social_network ) {

		if ( is_singular() ) {
			$short_link = get_post_meta( $post_id, 'bitly_url', true );
		} elseif ( is_front_page() ) {
			$short_link = bitly_get_blog_url();
		} else {
			$short_link = '';
		}

		if ( ! empty( $short_link ) ) {
			$permalink = $short_link;
		}
